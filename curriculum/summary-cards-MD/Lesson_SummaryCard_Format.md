...
- **Vertical alignment:** with text baseline at `cy` (`DV` 7.5 pt), draw the checkbox rect at `cy - 1.5` (bottom edge) so the 8pt box centers on the lowercase text body / x-height, not the baseline. (`cy - 6` or `cy - 7` sits visibly high — confirmed by rendering and visual review, not just eyeballing the code.)
...
- **Table-style layout, not inline dash format.** Term sits in a fixed-width left column; definition starts at a consistent indent for every row, like a two-column table.
- `def_indent` = the widest term's rendered width (`Helvetica-Bold` 7.5 pt) + 14 pt, computed per-lesson from that lesson's actual vocab list (**not a hardcoded global constant** — the column width should adapt so the term with the most characters still fits before the indent point).
- Term: `Helvetica-Bold` 7.5 pt, drawn at `x+8`
- Definition: `Helvetica` 7.5 pt, drawn at `x+8+def_indent`, wrapped to `col_w - 16 - def_indent`; wrapped lines continue at the same `def_indent`, not at a separate hanging-indent value
- Line height 11 pt, 6 pt gap between entries (more breathing room than the 3 pt used elsewhere, since rows are visually denser in a table layout)
